...
public:
    vector<string> removeComments(vector<string>& source) {
        ...
        for (const auto &str : source) src += str + "\r\n";
        
        while (end + 1 < src.size()) {
            bool in_comment = row_comment || block_comment;
            if (!in_comment && src[end] == '/' && src[end + 1] == '/') {
                dst += src.substr(beg, end++ - beg);
                row_comment = true;
            } else if (!in_comment && src[end] == '/' && src[end + 1] == '*') {
                dst += src.substr(beg, end++ - beg);
                block_comment = true;
            } else if (row_comment && src[end] == '\r' && src[end + 1] == '\n') {
                beg = end++;
                row_comment = false;
            } else if (block_comment && src[end] == '*' && src[end + 1] == '/') {
                beg = ++end + 1;
                block_comment = false;
            }
            ++end;
        ...
        vector<string> ans;
        beg = 0, end = 0;
        while (end + 1 < dst.size()) {
            if (dst[end] == '\r' && dst[end + 1] == '\n') {
                if (end - beg > 0) 
                    ans.push_back(dst.substr(beg, end - beg));
                beg = ++end + 1;
            }
            ++end;
        }
